...
name(name), pass(pass), userLimit(0)
{
    flags.nomsgout = true;
...
{
	if ((flags.topicset == true) && !isOperator(user))
...
flagChannel	Channel::getFlags() const
{
	return flags;
}
...
void	Channel::setFlag(unsigned char flag)
{
	...
	{
	case PRIVATE:
		this->flags._private = true;
		break;
	case SECRET:
		this->flags.secret = true;
		break;
	case MODERATED:
		this->flags.moderated = true;
		break;
	case INVITEONLY:
		this->flags.inviteonly = true;
		break;
	case TOPICSET:
		this->flags.topicset = true;
		break;
	case NOMSGOUT:
		this->flags.moderated = true;
		break;
...
void	Channel::removeFlag(unsigned char flag)
{
	...
	{
	case PRIVATE:
		this->flags._private = false;
		break;
	case SECRET:
		this->flags.secret = false;
		break;
	case MODERATED:
		this->flags.moderated = false;
		break;
	case INVITEONLY:
		this->flags.inviteonly = false;
		break;
	case TOPICSET:
		this->flags.topicset = false;
		break;
	case NOMSGOUT:
		this->flags.moderated = false;
		break;
...
void	Channel::displayChanInfo(const User &user)
{
	std::string	chanName = "";
	std::string	info = "";
	if ((flags.secret == true) && !containsNickname(user.getNickname()))
		return ;
	if ((flags._private == true) && !containsNickname(user.getNickname()))
		chanName = "*";
	else
	{
		chanName = name;
