				...
				const std::string &arg7,const std::string &arg8)
{
	...
	_send_from_server_log(msg, user, RPL_MOTD);
...
void	_send_from_server_log(std::string msg, const User &user, int id) {
	// for log on server
	if (id == RPL_MOTD)
		std::cerr << CYAN << "[SERVER->" + user.getNickname() + "]: " << GREEN << "[was send MOTD]" << RESET << "\n";
	else
		std::cerr << CYAN << "[SERVER->" + user.getNickname() + "]: " << GREEN << msg << RESET << "\n";
}